...
按照官方文档一点点敲,传送门:[地址](https://nest.nodejs.cn/recipes/prisma)

数据库连接采用的是`prisma`,按照他的官网使用了一下,感觉比`typeorm`好一些

通过掘金大佬神光的nest教程来辅助学习会好一些

流程图

![](https://cdn.jsdelivr.net/gh/ztao0916/image@main/img/202402161906893.png)

安装依赖包(可能会比较慢,等就好了,最慢20分钟安装完成)
...
会创建一个prisma目录,里面有个文件`schema.prisma`,指定你的数据库连接并包含数据库架构

还有一个env文件,存储环境变量

如何配置mysql数据库连接,参考官方文档: [传送门](https://prisma.nodejs.cn/getting-started/quickstart)

更新`schema.prisma`文件的数据库配置,更改内容如下
...
暂时没想到怎么从`.dev.env`文件导入环境变量,所以两个都引入,使用`envFilePath`加载数组的时候,同名环境变量数组靠前的文件会覆盖靠后的文件,`DATABASE_URL`需要定义在`.env`文件中

然后大致的一个操作流程:

```
1. 首先在schema.prisma中定义好数据模型
2. 然后执行迁移migrate命令创建数据库表,会在node_modules下生成了client代码,npx prisma migrate dev --name init
3. 使用@prisma/client软件包执行CRUD操作,会调用prisma generate命令
4. npx prisma studio会打开数据可视化,npx prisma migrate reset清空数据,重置数据库
```

具体代码如下

1. 将以下两个模型添加到你的 `schema.prisma` 文件中
...
```
nest g mo shared/prisma
nest g s shared/prisma
```

目的是为了抽象出 `Prisma Client API `以在服务中进行数据库查询
